...
| `PreToolUse` (Bash\|PowerShell) | worktree-guard | **`worktree-stash`** — BLOCK `git stash` (any subcommand) in a linked worktree (KIT-T233): `refs/stash` lives in the COMMON git dir, so every worktree shares ONE stack and parallel agents pop each other's entries. Baseline with `git diff > file` + checkout, or a WIP commit on the worktree branch. Escape: `[allow-stash: reason]` / `CLAUDE_KIT_ALLOW_STASH=1`. **`worktree-cwd`** — WARN (stderr, exit 0) when a mutating git command (`commit`/`merge`/`push`/`rebase`/`reset`/`checkout`/`switch`/`cherry-pick`/…) runs with an *implicit* cwd under `.claude/worktrees/` (KIT-T115): a drifted shell lands the commit on the agent branch. An explicit `git -C <dir>` / leading `cd <dir>` is a deliberate target and never warns. |
...
| `worktree-guard` | `worktree-stash` · `worktree-cwd` | path glob matched against the target checkout (`- "**"` disables a check for the repo); prefer the inline `[allow-stash:]` token for a one-off |
| `tree-liveness` | `tree-liveness` | path glob matched against the edited file (`- "**"` disables the check); prefer the inline `[allow-live-tree:]` token for a one-off |
